...
import { updateNonExtEmpAssesAction } from '../../../../../services/action/NonExecPrereqActions';
...
const EditNonExecPrereq = (props) => {
	const classes = useStyles();
	const dispatch = useDispatch();
	const { show, handler, assessment } = props;
	...
	useEffect(() => {
		setOpen(show);
		setInitialValues({ heading: assessment?.heading });
		if (assessment?.list) {
			setParams([...assessment?.list]);
		}
	}, [show]);
	...
	const onSubmit = (values) => {
		dispatch(
			updateNonExtEmpAssesAction(
				assessment._id,
				{ heading: values.heading, list: [...params] },
				() => {
					setSuccess(true);
					setTimeout(() => {
						setSuccess(false);
...
export default EditNonExecPrereq;
